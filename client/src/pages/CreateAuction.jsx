...
export const CreateAuction = () => {
  ...
  const handleAIDescription = async () => {
    ...
    try {
      ...
      if (res.data.provider === "fallback") {
        setError(
          "Gemini is currently unavailable (quota or config issue). A backup description was generated.",
        );
      } else {
        setError("");
      }
  ...
  const inputClasses =
    "w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-gray-900 text-sm placeholder:text-gray-300 focus:outline-none focus:ring-2 focus:ring-[#8d6f31]/40 focus:border-[#8d6f31] transition";
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div>
                ...
                <div className="flex gap-2 mb-1">
                  <button
                    type="button"
                    className={`text-xs font-semibold px-3 py-1.5 rounded-lg border border-[#d7c9ab] bg-[#efe3c7] text-[#6a542f] hover:bg-[#efe3c7] transition flex items-center gap-2 ${aiLoading ? "opacity-60 cursor-not-allowed" : ""}`}
                    onClick={handleAIDescription}
                    disabled={aiLoading}
                  >
                    {aiLoading ? (
                      <svg
                        className="animate-spin h-4 w-4 text-[#8d6f31]"
                        fill="none"
                        viewBox="0 0 24 24"
                      >
                        <circle
